 ...
 * @see Station
 */
public class FleetOfScooter extends FleetOfTrackable<Scooter> {

    /**
     * Constructeur qui initialise la liste de la flotte de véhicules
     */
    public FleetOfScooter() {
        super();
    }
     ...
     */
    public List<Scooter> around(Position vehiclePosition, int distanceMax) {
    ...
    public void generateFleet() {
        for (int i = 0; i < 5; i++) {
            this.fleetOfVehicles.add(new Scooter(
                    new Position(
                            Math.toIntExact(Math.round(Math.random() * 10)),
                            Math.toIntExact(Math.round(Math.random() * 10))
                    ),
                    Math.toIntExact(Math.round(Math.random() * 150))
            ));
        }
    }
